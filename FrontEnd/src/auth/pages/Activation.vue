<template>
  <q-page class="fit row wrap justify-center items-center content-center">
    <div class="col-6 col-xs-12 col-sm-6 col-lg-6 col-md-6 q-pl-xs q-pr-xs self-center">
      <q-card class="q-ma-none">
        <div class="text-center">
          <!-- <q-img src="~assets/logo.png" spinner-color="white" class="rounded-borders" style="max-width: 500px"></q-img> -->
        </div>
        <p class="text-weight-bolder text-center text-grey">Activar cuenta</p>
        <q-card-section v-if="activation?.isValid">
          <q-form class="q-gutter-md" ref="formActivation" @submit="doActivation">
            <q-input outlined clearable v-model="activation.usuario" type="email" label="Email" class="" readonly>
              <template v-slot:prepend>
                <q-icon name="email" color="primary" />
              </template>
            </q-input>
            <q-input outlined v-model="password" :type="isPwd ? 'password' : 'text'" label="Password" lazy-rules
            ...
            <q-input outlined v-model="repitepassword" :type="isPwd ? 'password' : 'text'" label="Repite password"
              lazy-rules :rules="[
            ...
            <q-card-actions>
              <div class="row full-width items-center">
                <div class="col-12">
                  <q-btn outline rounded size="md" color="primary" class="full-width" label="Activar" type="submit" />
                </div>
              </div>
            </q-card-actions>
          </q-form>
        ...
        <q-card-section>
          <p v-if="activation?.isValid" class="text-caption text-center text-weight-light text-grey">
            Crea su contraseña para accesar al sistema
          </p>
          <q-banner v-if="!activation?.isValid && activation?.mensaje != ''" rounded dense inline-actions
...
// import useHookUsuario from '../../app/hooks/Sistema/hookUsuarios';
